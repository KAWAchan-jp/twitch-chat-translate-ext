...
| 🎙️ **Субтитры речи стримера** | Автоматически распознает звук трансляции и показывает субтитры в реальном времени через Faster-Whisper (локальный сервер) или Groq API |
...
| ⚡ **Groq Whisper STT** | Облачное распознавание речи для быстрых и точных субтитров |
...
- **Облачное распознавание речи** — запускает Whisper Large-v3-Turbo / Large-v3 на высокоскоростной инфраструктуре Groq.
- **Есть бесплатный лимит** — ежемесячный сброс, бесплатный план $0.00/месяц. После достижения лимита или при ошибке в субтитрах показывается сообщение об ошибке.
...
- **Ошибка при сбое** — если сервер не запущен, произошла ошибка или тайм-аут, в субтитрах показывается сообщение об ошибке (автоматического fallback нет).
...
3. На странице Twitch убедитесь, что в футере панели показано **STT:** <span style="color:#c084fc">Faster</span>
   (если Groq тоже включен, щелчок переключает Faster ↔ Groq).
...
### Общие функции голосовых субтитров
...
- **Передача контекста** — последние фразы передаются как промпт, чтобы сохранять контекст распознавания (только Faster-Whisper).
...
- **Для голосовых субтитров**: нужен либо локальный сервер Faster-Whisper (NVIDIA GPU, рекомендуется от 4 ГБ VRAM), либо API-ключ Groq (подробности — в разделах соответствующих функций).
...
Чтобы использовать голосовые субтитры, включите на странице настроек либо **локальный сервер Faster-Whisper** (см. «Запуск сервера» выше), либо **API-ключ Groq**. Если ничего не настроено, при включении 🎤 в субтитрах появится сообщение об ошибке «движок распознавания речи не настроен».
...
Здесь можно сразу редактировать подсказки, передаваемые движку распознавания. Добавьте собственные имена, названия персонажей или термины через пробел, чтобы повысить точность распознавания. Ввод сохраняется автоматически и применяется со следующей фразы. Когда тема меняется, подсказки удобно быстро заменить.
...
| **STT:** | Движок распознавания речи (<span style="color:#c084fc">Faster</span> = Faster-Whisper локальный сервер / <span style="color:#f0971d">Groq</span> = Groq Whisper API) |

Изменения на странице настроек сразу отражаются в футере. **Щелчок по элементу футера переключает между включёнными движками** (ввод сообщений/голос: Google→DeepL→Gemini, STT: Faster→Groq — невключённые в настройках движки пропускаются).
...
| **STT:** | Faster → Groq (Groq появляется при заданном API-ключе) |
...
> Язык распознавания соответствует исходному языку в настройках канала. Если выбран `Auto detect`, движок распознавания определяет язык автоматически.
...
| Настройка | Описание |
|-----------|----------|
| **Подсказки распознавания по умолчанию** | Постоянные подсказки для всех каналов. Они добавляются перед временными подсказками панели 💡 (только Faster-Whisper) |
| **Размер шрифта субтитров** | Размер текста голосовых субтитров (14-56 px) |
| **Включить Faster-Whisper** | Распознавать речь через локальный сервер (GPU). Если сервер не запущен или произошла ошибка — сообщение об ошибке в субтитрах |
...
| **Включить Groq STT** | Распознавать речь через Groq Whisper API. При ошибке — сообщение об ошибке в субтитрах |
...
│   ├── content-whisper.js  # Content script (распознавание речи, субтитры, TTS; отправка аудио в Faster-Whisper/Groq)
...
Записанные аудиочанки отправляются через `background.js` (Service Worker) на `uv/server.py` (FastAPI + faster-whisper/CTranslate2).
Модели Large-серии выполняются на GPU локального ПК, результат возвращается в content script. При ошибке или тайм-ауте в субтитрах показывается ошибка.
...
Если Groq завершился ошибкой, в субтитрах показывается ошибка (автоматического fallback нет).

**VAD (Voice Activity Detection):**  
`AnalyserNode` отслеживает уровень громкости и запускает обработку, когда после речи продолжается заданный период тишины.
