...
Изменения на странице настроек сразу отражаются в футере. **Щелчок по элементу футера переключает между включёнными движками** (ввод сообщений/голос: Google→DeepL→Gemini, STT: Local⇄Groq — невключённые в настройках движки пропускаются).
...
| **STT:** | Local ⇄ Groq (если Groq включён) |
