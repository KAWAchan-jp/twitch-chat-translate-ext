...
- **フッター翻訳エンジンインジケーター** — パネル最下部に自分のメッセージ・音声字幕それぞれの翻訳エンジンを常時表示（Google / DeepL / Gemini）
...
| **Gemini を有効にする** | 音声字幕の翻訳に Gemini 2.0 Flash を使用（ゲーム用語・スラングに強い） |
| **Gemini API キー** | Google AI Studio で取得（無料）。音声字幕専用 |
